...
controller.listar = function (req, res) {
    Veiculo.find().exec().then(
        //callback do bem
        function (veiculos) {//todos os veikculos do vetor 
...
controller.atualizar = function (req, res) {
    ...
    Veiculo.findByIdAndUpdate({ _id: id }, req.body).exec().then(
        //callback do bem
        function (veiculo) {
            if (veiculo) {
...
controller.excluir = function (req, res) {
    const id = req.params.id;
    //capturamos o id a partir da url do veiculo
    Veiculo.findByIdAndDelete({ _id: id }).exec().then(
        //callback do bem
        function (veiculo) {
            if (veiculo) {
                res.sendStatus(204).end();
