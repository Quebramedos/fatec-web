const Classe = require('../models/Classe')(/*=construtor=*/);
...
controller.listar = function (req, res) {
    Classe.find().populate('proficiencia').exec().then(
        //callback do bem
        function (classes) {//todos os veikculos do vetor 
...
controller.atualizar = function (req, res) {
    ...
    Classe.findByIdAndUpdate({ _id: id }, req.body).exec().then(
        //callback do bem
        function (classe) {
            if (classe) {
...
controller.excluir = function (req, res) {
    ...
    Classe.findByIdAndDelete({ _id: id }).exec().then(
        //callback do bem
        function (classe) {
            if (classe) {
                res.sendStatus(204).end();
